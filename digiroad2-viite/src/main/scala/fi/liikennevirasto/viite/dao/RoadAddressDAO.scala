...
object RoadAddressDAO {
  ...
  implicit val getRoadAddress= new GetResult[RoadAddress]{
    def apply(r: PositionedResult) = {
      ...
      val validTo = r.nextDateOption.map(new DateTime(_))
  ...
  def queryById(ids: Set[Long], includeHistory: Boolean = false, includeTerminated: Boolean = false): List[RoadAddress] = {
    ...
    val query =
      s"""
        ...
        rn.ROAD_NAME as roadName
        from road_names rn, road_address ra cross join
        TABLE(SDO_UTIL.GETVERTICES(ra.geometry)) t cross join
        TABLE(SDO_UTIL.GETVERTICES(ra.geometry)) t2
        join lrm_position pos on ra.lrm_position_id = pos.id
        $where $historyFilter $terminatedFilter and t.id < t2.id and rn.ROAD_NUMBER = ra.ROAD_NUMBER AND rn.END_DATE IS NULL AND rn.VALID_TO IS NULL and
          ra.valid_to is null
      """
    queryList(query)
  }
